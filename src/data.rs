...
pub fn get_previous(matches: &ArgMatches) -> Result<(),failure::Error> {
    ...
    let n :usize = (matches.value_of("previous").unwrap_or("1"))
                            .parse()?;
...
pub fn get_next(matches: &ArgMatches) -> Result<(),failure::Error> {
    let n :usize = (matches.value_of("next").unwrap_or("1"))
                            .parse()?;
    //printprintln!("{}",n );
    if matches.is_present("local") {
        let table = get_table()?;
...
fn get_table() -> Result<Vec<(String,Date<Utc>)>,failure::Error> {
   ...
   let mut table: Vec<(_,_)> = table.into_iter().map(|(n,d)|{ 
...
fn previous_index(current_idx:usize, prev_arg:usize, table_len:usize) -> usize {
    let (table_len,current_idx,prev_arg) = (table_len as i32,current_idx as i32, prev_arg as i32);
